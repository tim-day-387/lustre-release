...
static
int mdc_xattr_common(struct obd_export *exp, const struct req_format *fmt,
			    ...
			    struct ptlrpc_request **request)
{
	...
	*request = NULL;
	...
	if (opcode == MDS_REINT) {
		...
		rec->sx_fsuid  = lustre_current_fsuid(idmap, NULL);
		rec->sx_fsgid  = lustre_current_fsgid(idmap, NULL);
